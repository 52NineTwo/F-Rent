...
<demo-card @click="GoTo_Detalis(content.name)" v-for="content in Supp_Data" :key="content.id" style="width: 49%;"
...
export default {
  ...
  methods: {
      Login_Data(){
          axios.get('user/dotpraise')
            .then(res => {
                if(res.status === 200) {
                    this.Supp_Data = res.data.data.list;
                }
            })
      },
      GoTo_Detalis(name){
